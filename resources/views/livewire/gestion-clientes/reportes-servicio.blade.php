...
<div class="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
    ...
    <div class="bg-white border border-gray-200 rounded-xl shadow-sm mb-5 overflow-hidden">
        ...
        <div class="p-4 grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-6 gap-3">

            <div class="xl:col-span-2 space-y-1">
                <label class="block text-[9px] font-black text-gray-400 uppercase tracking-widest">Buscar cliente o folio</label>
                <div class="relative">
            ...
            <div class="space-y-1">
                <label class="block text-[9px] font-black text-gray-400 uppercase tracking-widest">Tipo de Reporte</label>
                <select wire:model.live="filtroTipo"
                        class="w-full bg-gray-50 border border-gray-200 rounded-lg text-xs font-bold uppercase py-2.5 px-3 focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-400">
            ...
            <div class="space-y-1">
                <label class="block text-[9px] font-black text-gray-400 uppercase tracking-widest">Sucursal</label>
            ...
            <div class="space-y-1">
                ...
                <div class="flex gap-2">
                    <input type="month" wire:model.live="filtroPeriodo"
                           class="flex-1 bg-gray-50 border border-gray-200 rounded-lg text-xs font-bold py-2.5 px-3 focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-400">
                    <button wire:click="limpiarFiltros"
                            class="px-3 py-2.5 bg-white border border-gray-200 text-gray-400 rounded-lg hover:bg-red-50 hover:text-red-500 hover:border-red-200 transition-all text-xs" title="Limpiar filtros">
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-indigo-50/20 transition-colors group">
                        ...
                        <td class="px-5 py-4 whitespace-nowrap">
                            <p class="text-sm font-black text-indigo-600 tracking-tight font-mono">{{ $rep['folio'] }}</p>
                            <p class="text-[10px] text-gray-400 font-medium mt-0.5">{{ $rep['fecha_apertura'] }}</p>
                            @if($dias >= 3 && $rep['estado'] !== 'Cerrado')
                            <span class="inline-flex items-center gap-1 text-[9px] font-black text-red-600 bg-red-50 border border-red-100 px-1.5 py-0.5 rounded-md mt-1">
                                <i class="ri-alarm-warning-line"></i> {{ $dias }}d sin cerrar
                            </span>
                            @elseif($dias >= 1 && $rep['estado'] !== 'Cerrado')
                            <span class="inline-flex items-center gap-1 text-[9px] font-black text-amber-600 bg-amber-50 border border-amber-100 px-1.5 py-0.5 rounded-md mt-1">
                        ...
                        <td class="px-5 py-4">
                            ...
                            <p class="text-[9px] text-gray-400 mt-1 uppercase font-bold">Por: {{ $rep['quien_reporto'] }}</p>
                            @endif
                        </td>

                        {{-- Cliente / Servicio --}}
                        <td class="px-5 py-4">
                            <p class="text-xs font-black text-gray-900 uppercase tracking-tight">{{ $rep['cliente'] }}</p>
                            <p class="text-[10px] text-indigo-500 font-bold uppercase mt-0.5">{{ $rep['servicio_actual'] }}</p>
                        </td>
                        ...
                        <td class="px-5 py-4">
                            ...
                            <div class="flex items-center gap-1.5 mt-1">
                                ...
                                <span class="text-[10px] font-bold text-gray-500 uppercase">{{ $rep['tecnico'] ?? 'Sin asignar' }}</span>
                            </div>
                        </td>

                        {{-- Estado --}}
                        <td class="px-5 py-4 text-center">
                            <span class="inline-flex items-center gap-1 px-2.5 py-1 {{ $est['class'] }} rounded-lg text-[9px] font-black uppercase tracking-widest">
                        ...
                        <td class="px-5 py-4 whitespace-nowrap text-right">
                            <div class="flex justify-end items-center gap-2">
                                <button wire:click="exportarIndividual('{{ $rep['folio'] }}')"
                                        class="p-2 text-gray-300 hover:text-red-500 hover:bg-red-50 rounded-lg transition-all" title="Exportar PDF">
                    ...
                    <tr>
                        <td colspan="6" class="px-6 py-20 text-center">
                            <div class="flex flex-col items-center gap-3">
                                ...
                                <p class="text-xs font-black text-gray-300 uppercase tracking-widest">Sin reportes con los filtros seleccionados</p>
                                <button wire:click="limpiarFiltros" class="text-[10px] font-bold text-indigo-400 hover:text-indigo-600 uppercase tracking-widest">
                                    Limpiar filtros
                                </button>
                            </div>
                        </td>
