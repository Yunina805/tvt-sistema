<div>
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            <h1 class="text-base font-black uppercase tracking-widest text-gray-900">Descuentos para Tarifas</h1>
        ...
        @if($modo === 'lista')
            <button wire:click="nuevo"
                ...
                <i class="ri-add-line"></i> Nuevo Descuento
            </button>
        @endif
    </div>
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <div class="bg-emerald-50 border border-emerald-100 rounded-2xl p-4 flex items-center gap-3">
            <div class="w-9 h-9 bg-emerald-100 rounded-xl flex items-center justify-center shrink-0">
                <i class="ri-percent-line text-emerald-600 text-base"></i>
            </div>
            <div>
        ...
        <div class="bg-blue-50 border border-blue-100 rounded-2xl p-4 flex items-center gap-3">
            <div class="w-9 h-9 bg-blue-100 rounded-xl flex items-center justify-center shrink-0">
                <i class="ri-coupon-3-line text-blue-600 text-base"></i>
            </div>
            <div>
        ...
        <div class="bg-amber-50 border border-amber-100 rounded-2xl p-4 flex items-center gap-3">
            ...
            <div>
                <p class="text-[9px] font-black uppercase tracking-widest text-amber-500">Vencidos</p>
        ...
        <div class="bg-red-50 border border-red-100 rounded-2xl p-4 flex items-center gap-3">
            ...
            <div>
                <p class="text-[9px] font-black uppercase tracking-widest text-red-400">Cancelados</p>
                <p class="text-lg font-black text-red-700">{{ $kpis['cancelados'] }}</p>
            </div>
        </div>
    </div>
    @endif
    ...
    <div class="bg-white border border-emerald-200 rounded-2xl shadow-sm mb-6 p-6">
        <div class="flex items-center justify-between mb-5">
            <div class="flex items-center gap-3">
                <div class="w-9 h-9 bg-emerald-100 rounded-xl flex items-center justify-center">
                    <i class="ri-percent-line text-emerald-600 text-lg"></i>
                </div>
                <div>
                    <p class="text-xs font-black uppercase tracking-widest text-gray-800">
                        {{ $modo === 'crear' ? 'Nuevo Descuento' : 'Editar Descuento' }}
                    </p>
        ...
        {{-- Descripción --}}
        <div class="mb-4">
            <label class="block text-[9px] font-black uppercase tracking-widest text-gray-500 mb-1.5">Descripción del Descuento <span class="text-red-500">*</span></label>
            <input wire:model="descripcion" type="text" placeholder="Ej. Descuento tercera edad, convenio empresa..."
                class="w-full border border-gray-200 rounded-xl px-3 py-2 text-xs text-gray-800 focus:outline-none focus:ring-2 focus:ring-emerald-300 @error('descripcion') border-red-300 @enderror">
            @error('descripcion') <p class="text-[9px] text-red-500 mt-1">{{ $message }}</p> @enderror
        </div>

        {{-- Porcentaje / Monto --}}
        <div class="grid grid-cols-2 gap-4 mb-4">
            <div>
                <label class="block text-[9px] font-black uppercase tracking-widest text-gray-500 mb-1.5">Porcentaje de Descuento <span class="text-gray-300">(%)</span></label>
                <div class="relative">
                    <input wire:model="porcentajeDescuento" type="number" step="0.01" min="0" max="100" placeholder="0.00"
                        class="w-full border border-gray-200 rounded-xl px-3 pr-8 py-2 text-xs text-gray-800 focus:outline-none focus:ring-2 focus:ring-emerald-300 @error('porcentajeDescuento') border-red-300 @enderror">
                    <span class="absolute right-3 top-1/2 -translate-y-1/2 text-xs text-gray-400 font-bold">%</span>
                </div>
                @error('porcentajeDescuento') <p class="text-[9px] text-red-500 mt-1">{{ $message }}</p> @enderror
            </div>
            <div>
                <label class="block text-[9px] font-black uppercase tracking-widest text-gray-500 mb-1.5">Monto de Descuento <span class="text-gray-300">($)</span></label>
                <div class="relative">
                    <span class="absolute left-3 top-1/2 -translate-y-1/2 text-xs text-gray-400 font-bold">$</span>
                    <input wire:model="montoDescuento" type="number" step="0.01" min="0" placeholder="0.00"
                        class="w-full border border-gray-200 rounded-xl pl-7 pr-3 py-2 text-xs text-gray-800 focus:outline-none focus:ring-2 focus:ring-emerald-300 @error('montoDescuento') border-red-300 @enderror">
                </div>
                @error('montoDescuento') <p class="text-[9px] text-red-500 mt-1">{{ $message }}</p> @enderror
            </div>
        </div>
    ...
    {{-- Panel de Filtros --}}
    @php $hayFiltros = $search || $filtroPrincipal || $filtroEstado; @endphp
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                <div class="sm:col-span-2">
                    ...
                    <div class="relative">
                        ...
                        <input wire:model.live.debounce.300ms="search" type="text" placeholder="Descripción del descuento..."
                ...
                <div>
                    <label class="block text-[9px] font-black uppercase tracking-widest text-gray-500 mb-1.5">Tarifa Asociada</label>
    ...
    <div class="bg-white border border-gray-200 rounded-2xl shadow-sm">
            ...
            <table class="w-full text-xs">
                <thead>
                    <tr class="border-b border-gray-100">
                        ...
                        <th class="px-4 py-3 text-right text-[9px] font-black uppercase tracking-widest text-gray-400">Descuento</th>
                ...
                <tbody class="divide-y divide-gray-50">
                    @forelse($descuentos as $desc)
                        ...
                        <tr class="hover:bg-gray-50 transition-colors">
                            <td class="px-4 py-3 font-mono text-gray-400 text-[9px]">DS-{{ str_pad($desc->id, 4, '0', STR_PAD_LEFT) }}</td>
                            <td class="px-4 py-3 font-semibold text-gray-800 max-w-xs">{{ $desc->descripcion }}</td>
                            <td class="px-4 py-3 text-gray-500 text-[10px]">{{ $desc->tarifaPrincipal?->nombre_comercial ?? '—' }}</td>
                            <td class="px-4 py-3 text-right text-[10px]">
                                @if($desc->porcentaje_descuento)
                                    <span class="font-bold text-violet-600">{{ $desc->porcentaje_descuento }}%</span>
                                @endif
                                @if($desc->monto_descuento)
                                    <span class="font-mono text-violet-600">${{ number_format($desc->monto_descuento, 2) }}</span>
                                @endif
                                @if(!$desc->porcentaje_descuento && !$desc->monto_descuento)
                                    <span class="text-gray-300">—</span>
                                @endif
                            </td>
                            <td class="px-4 py-3">
                                <span class="px-2 py-0.5 {{ $estadoColor }} rounded text-[8px] font-black uppercase">{{ $desc->estado }}</span>
                            </td>
                            <td class="px-4 py-3 text-gray-500 font-mono text-[10px]">
                                {{ $desc->fecha_habilitacion->format('d/m/Y') }}
                                @if($desc->fecha_termino)
                                    — {{ $desc->fecha_termino->format('d/m/Y') }}
                                @else
                                    <span class="text-gray-300">Indef.</span>
                                @endif
                            </td>
                            <td class="px-4 py-3 text-gray-500 text-[10px]">{{ $desc->usuario?->name ?? '—' }}</td>
                            <td class="px-4 py-3">
                                <div class="flex items-center justify-end gap-1">
                                    <button wire:click="editar({{ $desc->id }})"
                                        class="p-1.5 text-gray-400 hover:text-emerald-600 hover:bg-emerald-50 rounded-lg transition-colors">
                                        <i class="ri-edit-line text-sm"></i>
                                    </button>
                                    <button @click="$confirm('¿Eliminar este descuento?', () => $wire.eliminar({{ $desc->id }}), { icon: 'warning', confirmText: 'Sí, eliminar' })"
                    ...
                    @empty
                        <tr>
                            <td colspan="8" class="px-5 py-12 text-center">
                                <i class="ri-percent-line text-3xl text-gray-200 block mb-2"></i>
                                <p class="text-xs font-bold text-gray-400 uppercase tracking-wider">Sin descuentos registrados</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
        ...
        @if($descuentos->hasPages())
            <div class="px-5 py-3 border-t border-gray-100">{{ $descuentos->links() }}</div>
        @endif
    </div>
</div>
