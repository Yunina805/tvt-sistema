<div>
    ...
    <nav class="flex items-center gap-2 text-[9px] font-black text-gray-400 uppercase tracking-[0.2em] mb-6">
        ...
        <span>/</span>
        <span class="text-gray-600">Promociones Estacionales</span>
    </nav>

    {{-- Header --}}
    <div class="flex items-center justify-between mb-6">
        <div>
            <h1 class="text-base font-black uppercase tracking-widest text-gray-900">Promociones Estacionales</h1>
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <div class="bg-emerald-50 border border-emerald-100 rounded-2xl p-4 flex items-center gap-3">
            <div class="w-9 h-9 bg-emerald-100 rounded-xl flex items-center justify-center shrink-0">
                <i class="ri-gift-line text-emerald-600 text-base"></i>
            </div>
            <div>
    ...
    <div class="bg-white border border-emerald-200 rounded-2xl shadow-sm mb-6 p-6">
        <div class="flex items-center justify-between mb-5">
            <div class="flex items-center gap-3">
                <div class="w-9 h-9 bg-emerald-100 rounded-xl flex items-center justify-center">
                    <i class="ri-gift-line text-emerald-600 text-lg"></i>
                </div>
                <div>
                    <p class="text-xs font-black uppercase tracking-widest text-gray-800">
                        {{ $modo === 'crear' ? 'Nueva Promoción Estacional' : 'Editar Promoción' }}
                    </p>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-4">
            ...
            <div>
                ...
                <input wire:model="nombreComercial" type="text" placeholder="Ej. Promo Verano 2 meses gratis"
        ...
        <div class="mb-4">
            ...
            <textarea wire:model="descripcion" rows="2" placeholder="Condiciones y detalles de la promoción..."
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-4">
            <div>
                <label class="block text-[9px] font-black uppercase tracking-widest text-gray-500 mb-1.5">Porcentaje Descuento <span class="text-gray-300">(%)</span></label>
                <div class="relative">
            ...
            <div>
                <label class="block text-[9px] font-black uppercase tracking-widest text-gray-500 mb-1.5">Monto Descuento <span class="text-gray-300">($)</span></label>
                <div class="relative">
            ...
            <div>
                <label class="block text-[9px] font-black uppercase tracking-widest text-gray-500 mb-1.5">Precio Mensualidad (Promo) <span class="text-red-500">*</span></label>
                <div class="relative">
                    <span class="absolute left-3 top-1/2 -translate-y-1/2 text-xs text-gray-400 font-bold">$</span>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                <div class="sm:col-span-2">
                    ...
                    <div class="relative">
                        ...
                        <input wire:model.live.debounce.300ms="search" type="text" placeholder="Nombre de la promoción..."
                ...
                <div>
                    <label class="block text-[9px] font-black uppercase tracking-widest text-gray-500 mb-1.5">Tarifa Principal</label>
    ...
    <div class="bg-white border border-gray-200 rounded-2xl shadow-sm">
            ...
            <table class="w-full text-xs">
                <thead>
                    <tr class="border-b border-gray-100">
                        ...
                        <th class="px-4 py-3 text-right text-[9px] font-black uppercase tracking-widest text-gray-400">Precio Promo</th>
                ...
                <tbody class="divide-y divide-gray-50">
                    @forelse($promociones as $promo)
                        @php
                            $estadoColor = match($promo->estado) {
                        ...
                        <tr class="hover:bg-gray-50 transition-colors">
                            <td class="px-4 py-3 font-mono text-gray-400 text-[9px]">PR-{{ str_pad($promo->id, 4, '0', STR_PAD_LEFT) }}</td>
                            <td class="px-4 py-3 font-semibold text-gray-800">{{ $promo->nombre_comercial }}</td>
                            <td class="px-4 py-3 text-gray-500 text-[10px]">{{ $promo->tarifaPrincipal?->nombre_comercial ?? '—' }}</td>
                            <td class="px-4 py-3 text-right text-[10px]">
                                @if($promo->porcentaje_descuento)
                                    <span class="font-bold text-violet-600">{{ $promo->porcentaje_descuento }}%</span>
                                @elseif($promo->monto_descuento)
                                    <span class="font-mono text-violet-600">${{ number_format($promo->monto_descuento, 2) }}</span>
                                @else
                                    <span class="text-gray-300">—</span>
                                @endif
                            </td>
                            <td class="px-4 py-3 text-right font-mono font-black text-emerald-700">${{ number_format($promo->precio_mensualidad, 2) }}</td>
                            <td class="px-4 py-3">
                                <span class="px-2 py-0.5 {{ $estadoColor }} rounded text-[8px] font-black uppercase">{{ $promo->estado }}</span>
                            </td>
                            <td class="px-4 py-3 text-gray-500 font-mono text-[10px]">
                                {{ $promo->fecha_habilitacion->format('d/m/Y') }}
                                @if($promo->fecha_termino)
                                    — {{ $promo->fecha_termino->format('d/m/Y') }}
                                @else
                            ...
                            <td class="px-4 py-3">
                                <div class="flex items-center justify-end gap-1">
                                    ...
                                    <button @click="$confirm('¿Eliminar esta promoción?', () => $wire.eliminar({{ $promo->id }}), { icon: 'warning', confirmText: 'Sí, eliminar' })"
                    ...
                    @empty
                        <tr>
                            <td colspan="8" class="px-5 py-12 text-center">
                                <i class="ri-gift-line text-3xl text-gray-200 block mb-2"></i>
                                <p class="text-xs font-bold text-gray-400 uppercase tracking-wider">Sin promociones registradas</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
        ...
        @if($promociones->hasPages())
            <div class="px-5 py-3 border-t border-gray-100">{{ $promociones->links() }}</div>
        @endif
    </div>
</div>
