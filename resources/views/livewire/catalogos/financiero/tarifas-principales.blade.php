<div>
    ...
    <nav class="flex items-center gap-2 text-[9px] font-black text-gray-400 uppercase tracking-[0.2em] mb-6">
        ...
        <span>/</span>
        <span class="text-gray-600">Tarifas Principales</span>
    </nav>

    {{-- Header --}}
    <div class="flex items-center justify-between mb-6">
        <div>
            <h1 class="text-base font-black uppercase tracking-widest text-gray-900">Tarifas Principales</h1>
        ...
        @if($modo === 'lista')
            <button wire:click="nueva"
                ...
                <i class="ri-add-line"></i> Nueva Tarifa
            </button>
        @endif
    </div>
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <div class="bg-emerald-50 border border-emerald-100 rounded-2xl p-4 flex items-center gap-3">
            ...
            <div>
                <p class="text-[9px] font-black uppercase tracking-widest text-emerald-500">Total Tarifas</p>
        ...
        <div class="bg-blue-50 border border-blue-100 rounded-2xl p-4 flex items-center gap-3">
            <div class="w-9 h-9 bg-blue-100 rounded-xl flex items-center justify-center shrink-0">
                <i class="ri-file-add-line text-blue-600 text-base"></i>
            </div>
            <div>
                <p class="text-[9px] font-black uppercase tracking-widest text-blue-500">Vigentes Contratar</p>
                <p class="text-lg font-black text-blue-800">{{ $kpis['vigente_contratar'] }}</p>
            </div>
        </div>
        <div class="bg-indigo-50 border border-indigo-100 rounded-2xl p-4 flex items-center gap-3">
            <div class="w-9 h-9 bg-indigo-100 rounded-xl flex items-center justify-center shrink-0">
                <i class="ri-calendar-check-line text-indigo-600 text-base"></i>
            </div>
            <div>
                <p class="text-[9px] font-black uppercase tracking-widest text-indigo-500">Vigentes Mensualidad</p>
                <p class="text-lg font-black text-indigo-800">{{ $kpis['vigente_mensualidad'] }}</p>
            </div>
        </div>
        <div class="bg-red-50 border border-red-100 rounded-2xl p-4 flex items-center gap-3">
            ...
            <div>
                <p class="text-[9px] font-black uppercase tracking-widest text-red-400">Vencidas / Canceladas</p>
                <p class="text-lg font-black text-red-700">{{ $kpis['vencidas_canceladas'] }}</p>
            </div>
        </div>
    </div>
    @endif
    ...
    <div class="bg-white border border-emerald-200 rounded-2xl shadow-sm mb-6 p-6">
        <div class="flex items-center justify-between mb-5">
            <div class="flex items-center gap-3">
                <div class="w-9 h-9 bg-emerald-100 rounded-xl flex items-center justify-center">
                    <i class="ri-price-tag-3-line text-emerald-600 text-lg"></i>
                </div>
                <div>
                    <p class="text-xs font-black uppercase tracking-widest text-gray-800">
                        {{ $modo === 'crear' ? 'Nueva Tarifa Principal' : 'Editar Tarifa Principal' }}
                    </p>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-4">
            <div class="lg:col-span-2">
                ...
                <input wire:model="nombreComercial" type="text" placeholder="Ej. Internet 50 Mbps + TV Básica"
            ...
            <div>
                ...
                <select wire:model="estado"
                    class="w-full border border-gray-200 rounded-xl px-3 py-2 text-xs text-gray-800 focus:outline-none focus:ring-2 focus:ring-emerald-300 @error('estado') border-red-300 @enderror">
                    @foreach($estados as $val => $label)
                        <option value="{{ $val }}">{{ $label }}</option>
                    @endforeach
                </select>
                @error('estado') <p class="text-[9px] text-red-500 mt-1">{{ $message }}</p> @enderror
            </div>
        </div>
        ...
        <div class="mb-4">
            ...
            <textarea wire:model="descripcion" rows="2" placeholder="Descripción de la tarifa..."
    ...
    {{-- Panel de Filtros --}}
    @php $hayFiltros = $search || $filtroEstado; @endphp
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                <div class="sm:col-span-2">
                    ...
                    <div class="relative">
                        ...
                        <input wire:model.live.debounce.300ms="search" type="text" placeholder="Nombre comercial, servicio..."
                ...
                <div class="sm:col-span-2">
                    <label class="block text-[9px] font-black uppercase tracking-widest text-gray-500 mb-1.5">Estado de Tarifa</label>
            ...
            <table class="w-full text-xs">
                <thead>
                    <tr class="border-b border-gray-100">
                        ...
                        <th class="px-4 py-3 text-left text-[9px] font-black uppercase tracking-widest text-gray-400">Nombre Comercial</th>
                        ...
                        <th class="px-4 py-3 text-left text-[9px] font-black uppercase tracking-widest text-gray-400">Habilitación</th>
                        <th class="px-4 py-3 text-left text-[9px] font-black uppercase tracking-widest text-gray-400">Término</th>
                        <th class="px-4 py-3 text-left text-[9px] font-black uppercase tracking-widest text-gray-400">CRT</th>
                    ...
                    @forelse($tarifas as $tarifa)
                        @php
                            $estadoColor = match($tarifa->estado) {
                                'VIGENTE_CONTRATAR'   => 'bg-emerald-50 text-emerald-700',
                                'VIGENTE_MENSUALIDAD' => 'bg-blue-50 text-blue-700',
                                'VENCIDA'             => 'bg-amber-50 text-amber-700',
                                'CANCELADA'           => 'bg-red-50 text-red-600',
                                default               => 'bg-gray-100 text-gray-500',
                            };
                            $estadoLabel = match($tarifa->estado) {
                                'VIGENTE_CONTRATAR'   => 'V. Contratar',
                                'VIGENTE_MENSUALIDAD' => 'V. Mensualidad',
                                'VENCIDA'             => 'Vencida',
                                'CANCELADA'           => 'Cancelada',
                                default               => $tarifa->estado,
                            };
                        @endphp
                        <tr class="hover:bg-gray-50 transition-colors">
                            <td class="px-4 py-3 font-mono text-gray-400 text-[9px]">TP-{{ str_pad($tarifa->id, 4, '0', STR_PAD_LEFT) }}</td>
                            <td class="px-4 py-3">
                                <p class="font-semibold text-gray-800">{{ $tarifa->nombre_comercial }}</p>
                                @if($tarifa->descripcion)
                                    <p class="text-[9px] text-gray-400 mt-0.5 line-clamp-1">{{ $tarifa->descripcion }}</p>
                                @endif
                            </td>
                            <td class="px-4 py-3 text-right font-mono font-semibold text-gray-700">
                                ${{ number_format($tarifa->precio_instalacion, 2) }}
                            </td>
                            <td class="px-4 py-3 text-right font-mono font-black text-emerald-700">
                                ${{ number_format($tarifa->precio_mensualidad, 2) }}
                            </td>
                            <td class="px-4 py-3">
                                <span class="px-2 py-0.5 {{ $estadoColor }} rounded text-[8px] font-black uppercase whitespace-nowrap">
                                    {{ $estadoLabel }}
                                </span>
                            </td>
                            ...
                            <td class="px-4 py-3 text-gray-500 font-mono text-[10px]">
                                {{ $tarifa->fecha_termino ? $tarifa->fecha_termino->format('d/m/Y') : '—' }}
                            </td>
                            <td class="px-4 py-3">
                                @if($tarifa->folio_crt)
                                    <span class="text-[9px] text-gray-500">{{ $tarifa->folio_crt }}</span>
                                @else
                            ...
                            <td class="px-4 py-3">
                                <div class="flex items-center justify-end gap-1">
                                    ...
                                    <button @click="$confirm('¿Eliminar esta tarifa?', () => $wire.eliminar({{ $tarifa->id }}), { icon: 'warning', confirmText: 'Sí, eliminar' })"
                    ...
                    @empty
                        <tr>
                            <td colspan="9" class="px-5 py-12 text-center">
                                ...
                                <p class="text-xs font-bold text-gray-400 uppercase tracking-wider">Sin tarifas registradas</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
