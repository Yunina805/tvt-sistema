<div>
    <nav class="flex items-center gap-2 text-[9px] font-black text-gray-400 uppercase tracking-[0.2em] mb-6">
        ...
        <span>Servicios</span>
        <span>/</span>
        <span class="text-gray-600">Actividades · Fallas de Servicio</span>
    </nav>

    <div class="flex items-center justify-between mb-6">
        <div>
            <h1 class="text-base font-black uppercase tracking-widest text-gray-900">Actividades para Fallas de Servicio</h1>
        ...
        <button wire:click="nuevo"
            class="flex items-center gap-2 px-4 py-2 bg-rose-600 hover:bg-rose-700 text-white text-xs font-black uppercase tracking-widest rounded-xl transition-colors shadow-sm">
    ...
    <div class="grid grid-cols-2 gap-4 mb-6">
        <div class="bg-rose-50 rounded-2xl p-4 flex items-center gap-3 shadow-sm border border-rose-100">
            <div class="w-10 h-10 bg-rose-100 rounded-xl flex items-center justify-center shrink-0">
                <i class="ri-list-check-3 text-rose-600 text-lg"></i>
            </div>
            <div>
                <p class="text-[9px] font-black uppercase tracking-widest text-rose-500">Total Actividades</p>
                <p class="text-xl font-black text-rose-800">{{ $total }}</p>
            </div>
        </div>
        <div class="bg-red-50 rounded-2xl p-4 flex items-center gap-3 shadow-sm border border-red-100">
            <div class="w-10 h-10 bg-red-100 rounded-xl flex items-center justify-center shrink-0">
                <i class="ri-checkbox-circle-line text-red-500 text-lg"></i>
            </div>
            <div>
                <p class="text-[9px] font-black uppercase tracking-widest text-red-400">Activas</p>
                <p class="text-xl font-black text-red-700">{{ $activos }}</p>
            </div>
        </div>
    </div>
    @endif
    ...
    <div class="bg-white border border-gray-200 rounded-2xl shadow-sm overflow-hidden mb-6">
        <div class="flex items-center justify-between px-5 py-3.5 border-b border-gray-100 bg-gray-50">
            <div class="flex items-center gap-2.5">
                <div class="w-7 h-7 bg-rose-100 rounded-lg flex items-center justify-center">
                    <i class="ri-{{ $modo === 'crear' ? 'add' : 'edit' }}-line text-rose-600 text-sm"></i>
                </div>
        ...
        <form wire:submit="guardar" class="p-5">
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-4">
                <div class="sm:col-span-3">
                    ...
                    <input type="text" wire:model="nombre"
                        class="w-full border border-gray-200 rounded-xl px-3 py-2 text-xs font-semibold text-gray-700 focus:outline-none focus:ring-2 focus:ring-rose-300 focus:border-transparent placeholder-gray-300 uppercase"
                        placeholder="REVISIÓN DE SEÑAL TV...">
                ...
                <div>
                    ...
                    <select wire:model="servicioId"
                        class="w-full border border-gray-200 rounded-xl px-3 py-2 text-xs font-semibold text-gray-700 focus:outline-none focus:ring-2 focus:ring-rose-300 focus:border-transparent bg-white">
                ...
                <div class="flex items-end pb-1">
                    <label class="flex items-center gap-2 cursor-pointer select-none">
                        <div class="relative">
                            ...
                            <div class="w-9 h-5 bg-gray-200 rounded-full peer-checked:bg-rose-500 transition-colors"></div>
            ...
            <div class="flex items-center justify-end gap-3 pt-2 border-t border-dashed border-gray-100">
                ...
                <button type="submit"
                    class="px-6 py-2 bg-rose-600 hover:bg-rose-700 text-white text-xs font-black uppercase tracking-widest rounded-xl transition-colors shadow-sm flex items-center gap-2">
            ...
            <table class="w-full">
                ...
                <tbody class="divide-y divide-gray-50">
                    ...
                    <tr class="hover:bg-gray-50 transition-colors">
                        <td class="px-4 py-3">
                            <span class="text-[9px] font-black text-rose-600 bg-rose-50 px-2 py-0.5 rounded-lg uppercase tracking-widest">
                        ...
                        <td class="px-4 py-3">
                            @if($act->servicio)
                            <span class="text-[9px] font-bold text-rose-700 bg-rose-50 px-2 py-0.5 rounded-lg">{{ $act->servicio->nombre }}</span>
                            @else
                    ...
                    <tr>
                        <td colspan="7" class="px-4 py-12 text-center">
                            <div class="flex flex-col items-center gap-2">
                                ...
                                <button wire:click="limpiarFiltros" class="text-[9px] font-black uppercase tracking-widest text-rose-500 hover:underline mt-1">Limpiar filtros</button>
                                @endif
                            </div>
                        </td>
                    </tr>
