<div>
    ...
    <nav class="flex items-center gap-2 text-[9px] font-black text-gray-400 uppercase tracking-[0.2em] mb-6">
        ...
        <span>Servicios</span>
        <span>/</span>
        <span class="text-gray-600">Registro para Tarifas Adicionales</span>
    </nav>

    {{-- Header --}}
    <div class="flex flex-wrap items-center justify-between gap-4 mb-6">
        <div>
            <h1 class="text-base font-black uppercase tracking-widest text-gray-900">Servicios — Tarifas Adicionales</h1>
            <p class="text-[10px] font-bold text-gray-400 uppercase tracking-wider mt-0.5">Catálogo de Servicios · Tarifas Adicionales</p>
        </div>
        @if($modo === 'lista')
        <button wire:click="nuevo"
            class="flex items-center gap-2 px-4 py-2 bg-sky-600 hover:bg-sky-700 text-white text-xs font-black uppercase tracking-wider rounded-xl transition-colors shadow-sm shadow-sky-200">
        ...
        <div class="bg-white border border-gray-200 rounded-2xl p-4 shadow-sm">
            <div class="flex items-center gap-3">
                <div class="w-10 h-10 bg-sky-100 rounded-xl flex items-center justify-center flex-shrink-0">
                    <i class="ri-add-circle-line text-sky-600"></i>
                </div>
                <div>
    ...
    {{-- Formulario --}}
    @if($modo !== 'lista')
    <div class="bg-white border border-sky-200 rounded-2xl shadow-sm mb-6 p-6">
        <div class="flex items-center justify-between mb-5">
            <div class="flex items-center gap-3">
                <div class="w-9 h-9 bg-sky-100 rounded-xl flex items-center justify-center">
                    <i class="ri-add-circle-line text-sky-600 text-base"></i>
                </div>
                <div>
                    <p class="text-xs font-black uppercase tracking-widest text-gray-800">
                        {{ $modo === 'crear' ? 'Nuevo Servicio' : 'Editar Servicio' }}
                    </p>
                    <p class="text-[9px] font-bold text-gray-400 uppercase tracking-wider">Tarifas Adicionales</p>
                </div>
            </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div class="space-y-1.5">
                ...
                <input wire:model="nombre" type="text" placeholder="Nombre descriptivo del servicio"
                    class="w-full px-3 py-2.5 text-xs bg-white border {{ $errors->has('nombre') ? 'border-red-300' : 'border-gray-200' }} rounded-xl font-medium text-gray-700 placeholder:text-gray-300 focus:outline-none focus:ring-2 focus:ring-sky-200 focus:border-sky-300 transition-all">
            ...
            <div class="space-y-1.5">
                ...
                <select wire:model="tarifaAdicionalId"
                    class="w-full px-3 py-2.5 text-xs bg-white border {{ $errors->has('tarifaAdicionalId') ? 'border-red-300' : 'border-gray-200' }} rounded-xl font-medium text-gray-700 focus:outline-none focus:ring-2 focus:ring-sky-200 focus:border-sky-300 transition-all">
                    <option value="">— Selecciona una tarifa —</option>
                    @foreach($tarifasAdicionales as $ta)
                    <option value="{{ $ta->id }}">TA-{{ str_pad($ta->id, 4, '0', STR_PAD_LEFT) }} — {{ $ta->nombre_comercial }}</option>
                    @endforeach
                </select>
                @error('tarifaAdicionalId')<p class="text-[9px] font-bold text-red-500">{{ $message }}</p>@enderror
            </div>

            <div class="space-y-1.5">
                <label class="block text-[9px] font-black text-gray-400 uppercase tracking-widest">Estado</label>
                <select wire:model="activo"
                    class="w-full px-3 py-2.5 text-xs bg-white border border-gray-200 rounded-xl font-medium text-gray-700 focus:outline-none focus:ring-2 focus:ring-sky-200 focus:border-sky-300 transition-all">
        ...
        <div class="flex justify-end gap-3 mt-5">
            ...
            <button wire:click="guardar" wire:loading.attr="disabled"
                class="flex items-center gap-2 px-5 py-2 bg-sky-600 hover:bg-sky-700 text-white text-xs font-black uppercase tracking-wider rounded-xl transition-colors disabled:opacity-60">
            ...
            <table class="w-full text-xs">
                <thead>
                    <tr class="border-b border-gray-100 bg-gray-50/50">
                        ...
                        <th class="px-4 py-3 text-left text-[9px] font-black text-gray-400 uppercase tracking-widest">Tarifa Adicional Asociada</th>
