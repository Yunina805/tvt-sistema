<div>
    ...
    <nav class="flex items-center gap-2 text-[9px] font-black text-gray-400 uppercase tracking-[0.2em] mb-6">
        ...
        <span>Servicios</span>
        <span>/</span>
        <span class="text-gray-600">Registro para Fallas de Servicio</span>
    </nav>

    {{-- Header --}}
    <div class="flex flex-wrap items-center justify-between gap-4 mb-6">
        <div>
            <h1 class="text-base font-black uppercase tracking-widest text-gray-900">Servicios — Fallas de Servicio</h1>
            <p class="text-[10px] font-bold text-gray-400 uppercase tracking-wider mt-0.5">Catálogo de Servicios · Fallas Técnicas</p>
        </div>
        @if($modo === 'lista')
        <button wire:click="nuevo"
            class="flex items-center gap-2 px-4 py-2 bg-rose-600 hover:bg-rose-700 text-white text-xs font-black uppercase tracking-wider rounded-xl transition-colors shadow-sm shadow-rose-200">
        ...
        <div class="bg-white border border-gray-200 rounded-2xl p-4 shadow-sm">
            <div class="flex items-center gap-3">
                <div class="w-10 h-10 bg-rose-100 rounded-xl flex items-center justify-center flex-shrink-0">
                    <i class="ri-alarm-warning-line text-rose-600"></i>
                </div>
                <div>
        ...
        <div class="bg-white border border-gray-200 rounded-2xl p-4 shadow-sm">
            <div class="flex items-center gap-3">
                <div class="w-10 h-10 bg-amber-50 rounded-xl flex items-center justify-center flex-shrink-0">
                    <i class="ri-user-voice-line text-amber-500"></i>
                </div>
                <div>
                    <p class="text-[9px] font-black text-gray-400 uppercase tracking-widest leading-none mb-0.5">Por Cliente</p>
                    <p class="text-2xl font-black text-gray-900 leading-none">{{ $porCliente }}</p>
                </div>
            </div>
        </div>
        <div class="bg-white border border-gray-200 rounded-2xl p-4 shadow-sm">
            <div class="flex items-center gap-3">
                <div class="w-10 h-10 bg-indigo-50 rounded-xl flex items-center justify-center flex-shrink-0">
                    <i class="ri-building-2-line text-indigo-500"></i>
                </div>
                <div>
                    <p class="text-[9px] font-black text-gray-400 uppercase tracking-widest leading-none mb-0.5">Por Tu Visión</p>
                    <p class="text-2xl font-black text-gray-900 leading-none">{{ $porTuVision }}</p>
                </div>
            </div>
        </div>
    </div>
    ...
    {{-- Formulario --}}
    @if($modo !== 'lista')
    <div class="bg-white border border-rose-200 rounded-2xl shadow-sm mb-6 p-6">
        <div class="flex items-center justify-between mb-5">
            <div class="flex items-center gap-3">
                <div class="w-9 h-9 bg-rose-100 rounded-xl flex items-center justify-center">
                    <i class="ri-alarm-warning-line text-rose-600 text-base"></i>
                </div>
                <div>
                    <p class="text-xs font-black uppercase tracking-widest text-gray-800">
                        {{ $modo === 'crear' ? 'Nuevo Servicio de Falla' : 'Editar Servicio de Falla' }}
                    </p>
                    <p class="text-[9px] font-bold text-gray-400 uppercase tracking-wider">Fallas de Servicio</p>
                </div>
            </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div class="space-y-1.5">
                ...
                <input wire:model="nombre" type="text" placeholder="Nombre descriptivo del servicio"
                    class="w-full px-3 py-2.5 text-xs bg-white border {{ $errors->has('nombre') ? 'border-red-300' : 'border-gray-200' }} rounded-xl font-medium text-gray-700 placeholder:text-gray-300 focus:outline-none focus:ring-2 focus:ring-rose-200 focus:border-rose-300 transition-all">
            ...
            <div class="space-y-1.5">
                ...
                <select wire:model="quienReporta"
                    class="w-full px-3 py-2.5 text-xs bg-white border {{ $errors->has('quienReporta') ? 'border-red-300' : 'border-gray-200' }} rounded-xl font-medium text-gray-700 focus:outline-none focus:ring-2 focus:ring-rose-200 focus:border-rose-300 transition-all">
                    <option value="">— Selecciona —</option>
                    <option value="CLIENTE">Cliente</option>
                    <option value="TU_VISION">Tu Visión</option>
                </select>
                @error('quienReporta')<p class="text-[9px] font-bold text-red-500">{{ $message }}</p>@enderror
            </div>

            <div class="space-y-1.5">
                <label class="block text-[9px] font-black text-gray-400 uppercase tracking-widest">Estado</label>
                <select wire:model="activo"
                    class="w-full px-3 py-2.5 text-xs bg-white border border-gray-200 rounded-xl font-medium text-gray-700 focus:outline-none focus:ring-2 focus:ring-rose-200 focus:border-rose-300 transition-all">
        ...
        <div class="flex justify-end gap-3 mt-5">
            ...
            <button wire:click="guardar" wire:loading.attr="disabled"
                class="flex items-center gap-2 px-5 py-2 bg-rose-600 hover:bg-rose-700 text-white text-xs font-black uppercase tracking-wider rounded-xl transition-colors disabled:opacity-60">
    ...
    {{-- Panel de Filtros --}}
    @php $hayFiltros = $search || $filtroActivo !== '' || $filtroReportado; @endphp
        ...
        <button type="button" @click="openFilters = !openFilters"
            ...
            <div class="flex items-center gap-3">
                <div class="w-9 h-9 bg-rose-50 rounded-xl flex items-center justify-center shrink-0">
                    <i class="ri-equalizer-2-line text-rose-500 text-base"></i>
                </div>
                <div class="text-left">
                    <p class="text-xs font-black uppercase tracking-widest text-gray-700">Filtros de Búsqueda</p>
                    <p class="text-[9px] font-bold uppercase tracking-widest mt-0.5 {{ $hayFiltros ? 'text-rose-500' : 'text-gray-400' }}">
                ...
                <div class="sm:col-span-2">
                    ...
                    <div class="relative">
                        ...
                        <input wire:model.live.debounce.300ms="search" type="text" placeholder="Nombre del servicio..."
                            class="w-full pl-9 pr-3 py-2.5 text-xs border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-rose-200 focus:border-rose-300 transition-all">
                    </div>
                </div>
                <div>
                    <label class="block text-[9px] font-black uppercase tracking-widest text-gray-500 mb-1.5">Quien Reporta</label>
                    <select wire:model.live="filtroReportado"
                        class="w-full py-2.5 px-3 text-xs border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-rose-200 focus:border-rose-300 transition-all">
            ...
            <table class="w-full text-xs">
                <thead>
                    <tr class="border-b border-gray-100 bg-gray-50/50">
                        ...
                        <th class="px-4 py-3 text-left text-[9px] font-black text-gray-400 uppercase tracking-widest">Quien Reporta</th>
                ...
                <tbody class="divide-y divide-gray-50">
                        ...
                        <td class="px-4 py-3">
                            <div class="flex items-center justify-end gap-1">
                                <button wire:click="editar({{ $svc->id }})"
                                    class="p-1.5 text-gray-400 hover:text-rose-600 hover:bg-rose-50 rounded-lg transition-colors" title="Editar">
                    ...
                    <tr>
                        <td colspan="7" class="px-5 py-14 text-center">
                            <i class="ri-alarm-warning-line text-3xl text-gray-200 block mb-2"></i>
