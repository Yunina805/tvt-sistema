<div>
    ...
    <nav class="flex items-center gap-2 text-[9px] font-black text-gray-400 uppercase tracking-[0.2em] mb-6">
        ...
        <span>Servicios</span>
        <span>/</span>
        <span class="text-gray-600">Registro para Personal TV</span>
    </nav>

    {{-- Header --}}
    <div class="flex flex-wrap items-center justify-between gap-4 mb-6">
        <div>
            <h1 class="text-base font-black uppercase tracking-widest text-gray-900">Servicios — Personal Tu Visión</h1>
            <p class="text-[10px] font-bold text-gray-400 uppercase tracking-wider mt-0.5">Catálogo de Servicios · Asignación por Puesto</p>
        </div>
        @if($modo === 'lista')
        <button wire:click="nuevo"
            class="flex items-center gap-2 px-4 py-2 bg-violet-600 hover:bg-violet-700 text-white text-xs font-black uppercase tracking-wider rounded-xl transition-colors shadow-sm shadow-violet-200">
        ...
        <div class="flex items-center justify-between mb-5">
            <div class="flex items-center gap-3">
                <div class="w-9 h-9 bg-violet-100 rounded-xl flex items-center justify-center">
                    <i class="ri-user-3-line text-violet-600 text-base"></i>
                </div>
                <div>
                    <p class="text-xs font-black uppercase tracking-widest text-gray-800">
                        {{ $modo === 'crear' ? 'Nuevo Servicio de Personal' : 'Editar Servicio de Personal' }}
                    </p>
                    <p class="text-[9px] font-bold text-gray-400 uppercase tracking-wider">Personal Tu Visión</p>
                </div>
            </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div class="space-y-1.5">
                ...
                <input wire:model="nombre" type="text" placeholder="Nombre descriptivo del servicio"
                    class="w-full px-3 py-2.5 text-xs bg-white border {{ $errors->has('nombre') ? 'border-red-300' : 'border-gray-200' }} rounded-xl font-medium text-gray-700 placeholder:text-gray-300 focus:outline-none focus:ring-2 focus:ring-violet-200 focus:border-violet-300 transition-all">
                @error('nombre')<p class="text-[9px] font-bold text-red-500">{{ $message }}</p>@enderror
            </div>

            <div class="space-y-1.5">
                <label class="block text-[9px] font-black text-gray-400 uppercase tracking-widest">
                    A Quien Será Asignado (Puesto) <span class="text-red-400">*</span>
                </label>
                <select wire:model="puestoAsignado"
                    class="w-full px-3 py-2.5 text-xs bg-white border {{ $errors->has('puestoAsignado') ? 'border-red-300' : 'border-gray-200' }} rounded-xl font-medium text-gray-700 focus:outline-none focus:ring-2 focus:ring-violet-200 focus:border-violet-300 transition-all">
                    <option value="">— Selecciona un puesto —</option>
                    @foreach($puestos as $p)
                    <option value="{{ $p }}">{{ $p }}</option>
                    @endforeach
                </select>
                @error('puestoAsignado')<p class="text-[9px] font-bold text-red-500">{{ $message }}</p>@enderror
            </div>

            <div class="space-y-1.5">
                <label class="block text-[9px] font-black text-gray-400 uppercase tracking-widest">Estado</label>
                <select wire:model="activo"
                    class="w-full px-3 py-2.5 text-xs bg-white border border-gray-200 rounded-xl font-medium text-gray-700 focus:outline-none focus:ring-2 focus:ring-violet-200 focus:border-violet-300 transition-all">
    ...
    {{-- Panel de Filtros --}}
    @php $hayFiltros = $search || $filtroActivo !== '' || $filtroPuesto; @endphp
            ...
            <table class="w-full text-xs">
                <thead>
                    <tr class="border-b border-gray-100 bg-gray-50/50">
                        ...
                        <th class="px-4 py-3 text-left text-[9px] font-black text-gray-400 uppercase tracking-widest">A Quien Será Asignado</th>
                ...
                <tbody class="divide-y divide-gray-50">
                    ...
                    <tr class="hover:bg-gray-50 transition-colors">
                        ...
                        <td class="px-4 py-3">
                            ...
                            @else
                            <span class="text-gray-300 text-[9px] italic">Sin puesto asignado</span>
                            @endif
                        </td>
                    ...
                    <tr>
                        <td colspan="7" class="px-5 py-14 text-center">
                            <i class="ri-user-3-line text-3xl text-gray-200 block mb-2"></i>
