<div>
    <nav class="flex items-center gap-2 text-[9px] font-black text-gray-400 uppercase tracking-[0.2em] mb-6">
        ...
        <span>Servicios</span>
        <span>/</span>
        <span class="text-gray-600">Actividades · Tarifas Adicionales</span>
    </nav>

    <div class="flex items-center justify-between mb-6">
        <div>
            <h1 class="text-base font-black uppercase tracking-widest text-gray-900">Actividades para Tarifas Adicionales</h1>
        ...
        <button wire:click="nuevo"
            class="flex items-center gap-2 px-4 py-2 bg-sky-600 hover:bg-sky-700 text-white text-xs font-black uppercase tracking-widest rounded-xl transition-colors shadow-sm">
    ...
    <div class="grid grid-cols-2 gap-4 mb-6">
        <div class="bg-sky-50 rounded-2xl p-4 flex items-center gap-3 shadow-sm border border-sky-100">
            <div class="w-10 h-10 bg-sky-100 rounded-xl flex items-center justify-center shrink-0">
                <i class="ri-list-check-3 text-sky-600 text-lg"></i>
            </div>
            <div>
                <p class="text-[9px] font-black uppercase tracking-widest text-sky-500">Total Actividades</p>
                <p class="text-xl font-black text-sky-800">{{ $total }}</p>
            </div>
        </div>
        <div class="bg-cyan-50 rounded-2xl p-4 flex items-center gap-3 shadow-sm border border-cyan-100">
            <div class="w-10 h-10 bg-cyan-100 rounded-xl flex items-center justify-center shrink-0">
                <i class="ri-checkbox-circle-line text-cyan-600 text-lg"></i>
            </div>
            <div>
                <p class="text-[9px] font-black uppercase tracking-widest text-cyan-500">Activas</p>
                <p class="text-xl font-black text-cyan-800">{{ $activos }}</p>
            </div>
        </div>
    </div>
    @endif
    ...
    <div class="bg-white border border-gray-200 rounded-2xl shadow-sm overflow-hidden mb-6">
        <div class="flex items-center justify-between px-5 py-3.5 border-b border-gray-100 bg-gray-50">
            <div class="flex items-center gap-2.5">
                <div class="w-7 h-7 bg-sky-100 rounded-lg flex items-center justify-center">
                    <i class="ri-{{ $modo === 'crear' ? 'add' : 'edit' }}-line text-sky-600 text-sm"></i>
                </div>
        ...
        <form wire:submit="guardar" class="p-5">
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-4">
                <div class="sm:col-span-3">
                    ...
                    <input type="text" wire:model="nombre"
                        class="w-full border border-gray-200 rounded-xl px-3 py-2 text-xs font-semibold text-gray-700 focus:outline-none focus:ring-2 focus:ring-sky-300 focus:border-transparent placeholder-gray-300 uppercase"
                        placeholder="INSTALACIÓN DE SERVICIO ADICIONAL...">
                ...
                <div>
                    ...
                    <select wire:model="servicioId"
                        class="w-full border border-gray-200 rounded-xl px-3 py-2 text-xs font-semibold text-gray-700 focus:outline-none focus:ring-2 focus:ring-sky-300 focus:border-transparent bg-white">
                ...
                <div class="flex items-end pb-1">
                    <label class="flex items-center gap-2 cursor-pointer select-none">
                        <div class="relative">
                            ...
                            <div class="w-9 h-5 bg-gray-200 rounded-full peer-checked:bg-sky-500 transition-colors"></div>
            ...
            <div class="flex items-center justify-end gap-3 pt-2 border-t border-dashed border-gray-100">
                ...
                <button type="submit"
                    class="px-6 py-2 bg-sky-600 hover:bg-sky-700 text-white text-xs font-black uppercase tracking-widest rounded-xl transition-colors shadow-sm flex items-center gap-2">
        ...
        <button type="button" @click="openFilters = !openFilters"
            ...
            <div class="flex items-center gap-3">
                <div class="w-9 h-9 bg-sky-50 rounded-xl flex items-center justify-center shrink-0">
                    <i class="ri-equalizer-2-line text-sky-500 text-base"></i>
                </div>
                <div class="text-left">
                    <p class="text-xs font-black uppercase tracking-widest text-gray-700">Filtros de Búsqueda</p>
                    <p class="text-[9px] font-bold uppercase tracking-widest mt-0.5 {{ $hayFiltros ? 'text-sky-500' : 'text-gray-400' }}">
                ...
                <div class="sm:col-span-2">
                    ...
                    <div class="relative">
                        ...
                        <input type="text" wire:model.live.debounce.300ms="search" placeholder="Nombre de la actividad..."
                            class="w-full pl-9 pr-3 py-2.5 text-xs border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-sky-200 focus:border-sky-300 transition-all">
                    </div>
                </div>
                <div>
                    ...
                    <select wire:model.live="filtroPuesto"
                        class="w-full py-2.5 px-3 text-xs border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-sky-200 focus:border-sky-300 transition-all">
            ...
            <table class="w-full">
                ...
                <tbody class="divide-y divide-gray-50">
                    ...
                    <tr class="hover:bg-gray-50 transition-colors">
                        <td class="px-4 py-3">
                            <span class="text-[9px] font-black text-sky-600 bg-sky-50 px-2 py-0.5 rounded-lg uppercase tracking-widest">
                        ...
                        <td class="px-4 py-3">
                            @if($act->servicio)
                            <span class="text-[9px] font-bold text-sky-700 bg-sky-50 px-2 py-0.5 rounded-lg">{{ $act->servicio->nombre }}</span>
                            @else
                        ...
                        <td class="px-4 py-3">
                            <div class="flex items-center gap-1">
                                <button wire:click="editar({{ $act->id }})"
                                    class="p-1.5 text-gray-400 hover:text-sky-600 hover:bg-sky-50 rounded-lg transition-colors" title="Editar">
                    ...
                    <tr>
                        <td colspan="7" class="px-4 py-12 text-center">
                            <div class="flex flex-col items-center gap-2">
                                ...
                                <button wire:click="limpiarFiltros" class="text-[9px] font-black uppercase tracking-widest text-sky-500 hover:underline mt-1">Limpiar filtros</button>
                                @endif
                            </div>
                        </td>
                    </tr>
